...
func update(newActiveLayer, myIndex, scrollScale: float, scrollTime: float, baseZPosition: float):
	...
	var compactnessFactor = distance * 0.3;
	...
	update_background_layers(centerScrollScale, scrollTime, compactnessFactor, offsetFactor, scrollScale)
	update_foreground_layers(centerScrollScale, scrollTime, compactnessFactor, offsetFactor, scrollScale)
		

func update_background_layers(baseScale, scrollTime, compactnessFactor, offsetFactor, scrollScale):
	...
	for layerIndex in range(backgroundLayers.size()):
		...
		var layerScale = cumulativeScale - (layer.relativeScrollScale * (1 - compactnessFactor));
...
func update_foreground_layers(baseScale, scrollTime, compactnessFactor, offsetFactor, scrollScale):
	var cumulativeScale = baseScale;
